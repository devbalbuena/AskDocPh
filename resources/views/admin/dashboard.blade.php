        ...
        <div class="{{ $crisisReports > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100' }} rounded-2xl shadow-sm border p-6 flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <p class="text-2xl {{ $crisisReports > 0 ? 'text-red-600 font-bold' : 'font-bold text-gray-900' }}">{{ number_format($crisisReports) }}</p>
                    @if($crisisReports > 0)
                    <span class="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse inline-block" title="Pending crisis reports"></span>
                    @endif
                </div>
                <p class="{{ $crisisReports > 0 ? 'text-red-500' : 'text-gray-500' }} text-sm">Open Crisis Reports</p>
            </div>
        </div>
    </div>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200/50 hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-5 py-3 text-right">
                            <a href="{{ route('admin.doctor-applications.show', $app) }}" class="inline-block bg-white border border-green-600 text-green-600 text-sm px-3 py-1 rounded-lg hover:bg-green-50 transition-colors">View Details</a>
                        </td>
                    </tr>
                    @empty
...
new Chart(ctx, {
    ...
    options: {
        ...
        scales: {
            ...
            y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ca3af', stepSize: 1, precision: 0 }, beginAtZero: true }
        }
    }
});
</script>
